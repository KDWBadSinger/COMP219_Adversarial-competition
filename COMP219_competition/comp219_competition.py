# -*- coding: utf-8 -*-
"""COMP219 Assignment.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1EhaskoiDtc-DCpHciCOLB6c-gketnaKo
"""
...
# setup training parameters 训练参数
...
# judge cuda is available or not 是否启用CUDA加速
...
# define fully connected network 全连接层
class Net(nn.Module):
    def __init__(self):
...
#generate adversarial data, you can define your adversarial method 对抗性攻击代码，改动中间那一块
def adv_attack(model, X, y, device):
    ...
    mu = 0.3  # 动量因子
    delta = torch.zeros_like(X).to(device)
    grad_accum = torch.zeros_like(X).to(device)  # 用于存储累积梯度
    ...
    grad_accum = torch.zeros_like(X, device=device)

    with torch.enable_grad():
      for scale, num_iters in zip(scale_steps, scale_number):
        for _ in range(num_iters):
          ...
          model.zero_grad()
          loss.backward()
          grad_accum = mu * grad_accum + X_temp.grad.data / X_temp.grad.data.norm()
          delta = torch.clamp(delta + scale * X_temp.grad.sign(), -epsilon, epsilon)
